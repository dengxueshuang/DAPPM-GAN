...
class RadioWNet(nn.Module):
    ...
    def __init__(self, inputs=1, phase="firstU"):
        ...
        if inputs <= 3:
            ...
            self.middle00 = nn.Sequential(*[DAPPM(6, 6, 6) for _ in range(1)])
            self.layer0 = convrelu(6, 40, 5, 2, 2)
            self.middle0 = nn.Sequential(*[DAPPM(40, 40, 40) for _ in range(1)])


        self.layer1 = convrelu(40, 50, 5, 2, 2)
        self.middle1 = nn.Sequential(*[DAPPM(50, 50, 50) for _ in range(1)])
        self.layer10 = convrelu(50, 60, 5, 2, 1)
        self.middle10 = nn.Sequential(*[DAPPM(60, 60, 60) for _ in range(1)])
        self.layer2 = convrelu(60, 100, 5, 2, 2)
        self.middle2 = nn.Sequential(*[DAPPM(100, 100, 100) for _ in range(1)])
        self.layer20 = convrelu(100, 100, 3, 1, 1)
        self.middle20 = nn.Sequential(*[DAPPM(100, 100, 100) for _ in range(1)])
        self.layer3 = convrelu(100, 150, 5, 2, 2)
        self.middle3 = nn.Sequential(*[DAPPM(150, 150, 150) for _ in range(1)])
        self.layer4 = convrelu(150, 300, 5, 2, 2)
        self.middle4 = nn.Sequential(*[DAPPM(300, 300, 300) for _ in range(1)])
        self.layer5 = convrelu(300, 500, 5, 2, 2)
        self.middle5 = nn.Sequential(*[DAPPM(500, 500, 500) for _ in range(1)])
    ...
    def forward(self, input):
        ...
        if self.phase == "firstU":
            ...
            layerp00 = self.middle00(layer00)

            layer0 = self.layer0(layerp00)
            layerp0 = self.middle0(layer0)

            layer1 = self.layer1(layerp0)
            layerp1 = self.middle1(layer1)

            layer10 = self.layer10(layerp1)
            layerp10 = self.middle10(layer10)

            layer2 = self.layer2(layerp10)
            layerp2 = self.middle2(layer2)

            layer20 = self.layer20(layerp2)
            layerp20 = self.middle20(layer20)
            layer3 = self.layer3(layerp20)
            layerp3 = self.middle3(layer3)
            layer4 = self.layer4(layerp3)
            layerp4 = self.middle4(layer4)
            layer5 = self.layer5(layerp4)
            layerp5 = self.middle5(layer5)

            layer4u = self.conv_up5(layerp5)
